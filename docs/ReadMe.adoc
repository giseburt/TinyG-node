...
Class TinyG::
  ...
  ** *Returns:* _nothing_
  ** +_filename_or_stdin_+: Either a path name (in a string) or a `readStream` object (such as `process.stdin`).
  ** +_done_callback_+ (_optional_): A function for the TinyG object to call when the file has finished sending. This will only be called after all lines have been sent AND `stat` has gone to 3 (movement stopped), 4 (program end via `M2` or `M30`), or 6 (alarm).
